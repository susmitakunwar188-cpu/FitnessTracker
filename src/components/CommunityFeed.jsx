        ...
        {loading ? (
          ...
          <div className="grid grid-cols-2 gap-3">
            {feed.map((post, idx) => {
              ...
              return (
                <div key={id} className="overflow-hidden rounded-xl bg-bg-dark/60 border border-border-pink/40 flex flex-col">
                  {post.imageUrl && (
                    <img
                      src={post.imageUrl}
                      alt={post.action || ''}
                  ...
                  {post.action && (
                    <p className="px-2.5 py-2 text-xs text-text-primary leading-snug line-clamp-3">{post.action}</p>
                  )}
                </div>
              );
            })}
