...
const getDerivedProfileGoal = (user) => {
  const weight = Number(user?.weight) || 0;
  const height = Number(user?.height) || 0;
  const savedStatus = String(user?.status || '').toLowerCase();
  const savedGoal = String(user?.goal || '').toLowerCase();

  if (savedGoal) return savedGoal;

  if (weight > 0 && height > 0) {
    const heightInMeters = height / 100;
    const bmi = weight / (heightInMeters * heightInMeters);

    if (bmi < 18.5) return 'gain healthy weight';
    if (bmi < 25) return 'maintain healthy weight';
    if (bmi < 30) return 'lose weight';
    return 'lose weight';
  }

  if (savedStatus.includes('under')) return 'gain healthy weight';
  if (savedStatus.includes('over') || savedStatus.includes('obese')) return 'lose weight';
...
const getGoals = (user, activityFactor = 1.375) => {
  ...
  if (w > 0 && h > 0 && age > 0) {
    ...
    } else if (goalText.includes('lose') || goalText.includes('loss') || status.includes('over') || status.includes('obese') || bmi >= 25) {
      calories -= 300;
    ...
    if (goalText.includes('gain')) {
      calories += 300;
      protein = Math.round(w * 2.1);
    } else if (goalText.includes('lose') || goalText.includes('loss')) {
      calories -= 300;
      protein = Math.round(w * 1.9);
    } else {
      protein = Math.round(w * 1.8);
